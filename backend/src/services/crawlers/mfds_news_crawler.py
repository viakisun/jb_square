...
class MFDSNewsCrawler(BaseCrawler):
    ...
    식품의약품안전처 RSS 피드를 수집하고 원본 페이지를 크롤링하여
    ...
    async def execute(
        ...
        try:
            ...
            result = {
                ...
                "success": self.status["success"],
                "failed": self.status["failed"],
                "message": f"식약처 RSS 크롤링 완료: {self.status['success']}개 저장, {self.status['failed']}개 실패"
            }
            ...
            logger.exception(f"식약처 RSS 크롤링 중 오류: {e}")
    ...
    def _fetch_detail_page(self, url: str) -> Dict:
        ...
        try:
            ...
            logger.warning(f"식약처 페이지 크롤링 실패 ({url}): {e}")
            return {
                'content_html': '',
                'attachments': [],
                'images': []
