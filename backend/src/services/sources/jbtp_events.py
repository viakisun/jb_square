...
class JBTPEventsAdapter(JBTPBaseAdapter):
    ...
    async def get_notices_preview(
        ...
        try:
            ...
            board_url = config.get('board_url') or config.get('url', 'https://www.jbtp.or.kr/board/list.jbtp?boardId=BBS_0000003')
            ...
            while len(notices_preview) < count and page <= MAX_PAGES:
                ...
                for row in rows:
                    ...
                    # 날짜 필터링 (posted_date >= cutoff OR deadline >= now)
            ...
            print(f"[JBTPEventsAdapter] get_notices_preview 오류: {str(e)}")
            import traceback
            traceback.print_exc()
            return []
