...
4. **Crawlers**: Modular architecture with BaseCrawler abstract class and specialized implementations for JBTP, NTIS, BIZINFO, and BI Centers
...
- **BaseCrawler** ([backend/src/services/crawlers/base_crawler.py](backend/src/services/crawlers/base_crawler.py)): Abstract base class providing status tracking, WebSocket events, keyword matching, and date parsing utilities
- **CrawlerManager** ([backend/src/services/crawler_manager.py](backend/src/services/crawler_manager.py)): Orchestrates crawler instances and manages execution (note: still 24KB, requires further refactoring)
- **Specialized Crawlers**: JBTP, NTIS (API-based), BIZINFO, BI Center crawlers inherit from BaseCrawler
...
SvelteKit file-based routing in [frontend/src/routes/](frontend/src/routes/):
- `/`: Dashboard home page
...
The crawler system underwent major refactoring to modular architecture. The original monolithic [crawler_manager.py](backend/src/services/crawler_manager.py) (2416 lines) has been partially refactored. Each crawler now inherits from BaseCrawler, but crawler_manager.py still requires further decomposition.
...
- [backend/tests/test_crawling.py](backend/tests/test_crawling.py): WebSocket crawling tests
...
1. **crawler_manager.py**: Still 24KB, needs further modularization
