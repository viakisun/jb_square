 ...
 * 📋 보건복지부 뉴스 페이지
 *
 * 보건복지부 보도자료 및 바이오 산업 관련 뉴스를 보여주는 페이지입니다.
 *
 * **주요 기능:**
 * - 보건복지부 뉴스(source_id='source:news:mohw') 필터링
 * - 검색 기능 (제목, 내용, 기관명)
 ...
 * 보건복지부 뉴스 페이지 컴포넌트
 */
export default function MOHWNewsPage() {
  /**
   * 검색 모드 상태
  ...
  /**
   * useNotices 훅 - 보건복지부 뉴스 데이터 가져오기
   *
   * source_id: 'source:news:mohw' - 보건복지부 뉴스만 필터링
   ...
   *
   * additionalFilters: source_id를 'source:news:mohw'로 고정하여
   * 보건복지부 뉴스 내에서만 검색하도록 제한
   */
  const {
    searchQuery,          // 현재 검색어
  ...
  return (
    <>
      {/* SEO를 위한 페이지 메타데이터 */}
      <Head>
        <title>보건복지부 뉴스 - JB SQUARE</title>
        <meta name="description" content="보건복지부 보도자료 및 바이오 산업 관련 뉴스" />
      </Head>

      {/* 공통 레이아웃 사용 */}
      <NoticePageLayout
        pageTitle="보건복지부 뉴스"
        pageSubtitle="보건복지부 보도자료 및 바이오 산업 관련 뉴스"
        breadcrumbCurrent="보건복지부 뉴스"
      >
        {/* 검색바 */}
        <div className="mb-8">
          <SearchBar
